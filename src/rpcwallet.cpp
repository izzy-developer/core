...
Value getnewaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
                ...
                "\nReturns a new IZZY address for receiving payments.\n"
                "If 'account' is specified (recommended), it is added to the address book \n"
                "so payments received with the address will be credited to 'account'.\n"
                "\nArguments:\n"
                "1. \"account\"        (string, optional) The account name for the address to be linked to. if not provided, the default account \"\" is used. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created if there is no account by the given name.\n"
                "\nResult:\n"
                "\"izzyaddress\"    (string) The new izzy address\n"
                "\nExamples:\n" +
...
Value getaccountaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
                ...
                "\nReturns the current IZZY address for receiving payments to this account.\n"
                "\nArguments:\n"
                "1. \"account\"       (string, required) The account name for the address. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created and a new address created  if there is no account by the given name.\n"
                "\nResult:\n"
                "\"izzyaddress\"   (string) The account izzy address\n"
                "\nExamples:\n" +
...
Value getrawchangeaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
                "getrawchangeaddress\n"
                "\nReturns a new IZZY address, for receiving change.\n"
...
Value setaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
                "setaccount \"izzyaddress\" \"account\"\n"
                "\nSets the account associated with the given address.\n"
                "\nArguments:\n"
                "1. \"izzyaddress\"  (string, required) The izzy address to be associated with an account.\n"
    ...
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid IZZY address");
...
Value getaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
                "getaccount \"izzyaddress\"\n"
                "\nReturns the account associated with the given address.\n"
                "\nArguments:\n"
                "1. \"izzyaddress\"  (string, required) The izzy address for account lookup.\n"
                "\nResult:\n"
...
Value getaddressesbyaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
                ...
                "  \"izzyaddress\"  (string) a izzy address associated with the given account\n"
                "  ,...\n"
                "]\n"
                "\nExamples:\n" +
...
Value fundvault(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
                ...
                "1. \"manager_address\"  (string, required) The izzy address owned by the vault manager.\n"
                "2. \"amount\"      (numeric, required) The amount in IZZY to send. eg 0.1\n"
    ...
    if (!managerAddress.IsValid() || !managerAddress.GetKeyID(managerKeyID))
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Funding failed: Invalid manager IZZY address");

    CKeyID ownerKeyID;
    if (!ownerAddress.IsValid() || !ownerAddress.GetKeyID(ownerKeyID))
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Funding failed: Invalid owner IZZY address");
...
Value reclaimvaultfunds(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
                ...
                HelpRequiringPassphrase() +
                "\nArguments:\n"
                "1. \"izzyaddress\"  (string, required) The izzy address of your choosing to send to.\n"
                "2. \"amount\"      (numeric, required) The amount in IZZY to move. eg 0.1\n"
                "\nResult:\n"
...
Value removevault(const Array& params, bool fHelp)
{
    ...
    CKeyID managerKeyID;
    if (!managerAddress.IsValid() || !managerAddress.GetKeyID(managerKeyID))
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Vault Registry Failed: Invalid manager IZZY address");

    CKeyID ownerKeyID;
    if (!ownerAddress.IsValid() || !ownerAddress.GetKeyID(ownerKeyID))
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Vault Registry Failed: Invalid owner IZZY address");
...
Value sendtoaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
                "sendtoaddress \"izzyaddress\" amount ( \"comment\" \"comment-to\" )\n"
                "\nSend an amount to a given address. The amount is a real and is rounded to the nearest 0.00000001\n" +
                HelpRequiringPassphrase() +
                "\nArguments:\n"
                "1. \"izzyaddress\"  (string, required) The izzy address to send to.\n"
                "2. \"amount\"      (numeric, required) The amount in IZZY to send. eg 0.1\n"
                "3. \"comment\"     (string, optional) A comment used to store what the transaction is for. \n"
                "                             This is not part of the transaction, just kept in your wallet.\n"
                "4. \"comment-to\"  (string, optional) A comment to store the name of the person or organization \n"
...
Value listaddressgroupings(const Array& params, bool fHelp)
{
    if (fHelp)
        throw runtime_error(
                ...
                "  [\n"
                "    [\n"
                "      \"izzyaddress\",     (string) The izzy address\n"
                "      amount,                 (numeric) The amount in IZZY\n"
...
Value signmessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
                "signmessage \"izzyaddress\" \"message\" \"input_format\" \"output_format\"\n"
                "\nSign a message with the private key of an address" +
                HelpRequiringPassphrase() + "\n"
                                            "\nArguments:\n"
                                            "1. \"izzyaddress\"    (string, required) The izzy address to use for the private key.\n"
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
                "getreceivedbyaddress \"izzyaddress\" ( minconf )\n"
                "\nReturns the total amount received by the given izzyaddress in transactions with at least minconf confirmations.\n"
                "\nArguments:\n"
                "1. \"izzyaddress\"  (string, required) The izzy address for transactions.\n"
                "2. minconf             (numeric, optional, default=1) Only include transactions confirmed at least this many times.\n"
                "\nResult:\n"
                "amount   (numeric) The total amount in IZZY received at this address.\n"
                "\nExamples:\n"
...
Value getreceivedbyaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
                ...
                "2. minconf          (numeric, optional, default=1) Only include transactions confirmed at least this many times.\n"
                "\nResult:\n"
                "amount              (numeric) The total amount in IZZY received for this account.\n"
                "\nExamples:\n"
...
Value movecmd(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 5)
        throw runtime_error(
                ...
                "\nExamples:\n"
                "\nMove 0.01 IZZY from the default account to the account named tabby\n" +
                HelpExampleCli("move", "\"\" \"tabby\" 0.01") +
                "\nMove 0.01 IZZY timotei to akiko with a comment and funds have 6 confirmations\n" + HelpExampleCli("move", "\"timotei\" \"akiko\" 0.01 6 \"happy birthday!\"") +
...
Value sendfrom(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw runtime_error(
                "sendfrom \"fromaccount\" \"toizzyaddress\" amount ( minconf \"comment\" \"comment-to\" )\n"
                "\nSent an amount from an account to a izzy address.\n"
                "The amount is a real and is rounded to the nearest 0.00000001." +
                HelpRequiringPassphrase() + "\n"
                                            "\nArguments:\n"
                                            "1. \"fromaccount\"       (string, required) The name of the account to send funds from. May be the default account using \"\".\n"
                                            "2. \"toizzyaddress\"  (string, required) The izzy address to send funds to.\n"
                                            "3. amount                (numeric, required) The amount in IZZY. (transaction fee is added on top).\n"
                                            ...
                                            "\"transactionid\"        (string) The transaction id.\n"
                                            "\nExamples:\n"
                                            "\nSend 0.01 IZZY from the default account to the address, must have at least 1 confirmation\n" +
...
Value sendmany(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
                ...
                HelpRequiringPassphrase() + "\n"
                                            ...
                                            "    {\n"
                                            "      \"address\":amount   (numeric) The izzy address is the key, the numeric amount in IZZY is the value\n"
                                            "      ,...\n"
                                            "    }\n"
    ...
    BOOST_FOREACH (const Pair& s, sendTo) {
        ...
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid IZZY address: ") + s.name_);
...
Value addmultisigaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3) {
        string msg = "addmultisigaddress nrequired [\"key\",...] ( \"account\" )\n"
                     ...
                     "Each key is a IZZY address or hex-encoded public key.\n"
                     "If 'account' is specified, assign address to that account.\n"

                     "\nArguments:\n"
                     "1. nrequired        (numeric, required) The number of required signatures out of the n keys or addresses.\n"
                     "2. \"keysobject\"   (string, required) A json array of izzy addresses or hex-encoded public keys\n"
                     "     [\n"
                     "       \"address\"  (string) izzy address or hex-encoded public key\n"
                     "       ...,\n"
                     "     ]\n"
                     "3. \"account\"      (string, optional) An account to assign the addresses to.\n"

                     "\nResult:\n"
                     "\"izzyaddress\"  (string) A izzy address associated with the keys.\n"
...
Value listreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 3)
        throw runtime_error(
                ...
                "    \"amount\" : x.xxx,                  (numeric) The total amount in IZZY received by the address\n"
...
Value listtransactions(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 4)
        throw runtime_error(
                ...
                "    \"address\":\"izzyaddress\",    (string) The izzy address of the transaction. Not present for \n"
                ...
                "    \"amount\": x.xxx,          (numeric) The amount in IZZY. This is negative for the 'send' category, and for the\n"
                "                                         'move' category for moves outbound. It is positive for the 'receive' category,\n"
                "                                         and for the 'move' category for inbound funds.\n"
                "    \"vout\" : n,               (numeric) the vout value\n"
                "    \"fee\": x.xxx,             (numeric) The amount of the fee in IZZY. This is negative and only available for the \n"
...
Value listsinceblock(const Array& params, bool fHelp)
{
    if (fHelp)
        throw runtime_error(
                ...
                "    \"address\":\"izzyaddress\",    (string) The izzy address of the transaction. Not present for move transactions (category = move).\n"
                "    \"category\":\"send|receive\",     (string) The transaction category. 'send' has negative amounts, 'receive' has positive amounts.\n"
                "    \"amount\": x.xxx,          (numeric) The amount in IZZY. This is negative for the 'send' category, and for the 'move' category for moves \n"
                "                                          outbound. It is positive for the 'receive' category, and for the 'move' category for inbound funds.\n"
                "    \"vout\" : n,               (numeric) the vout value\n"
                "    \"fee\": x.xxx,             (numeric) The amount of the fee in IZZY. This is negative and only available for the 'send' category of transactions.\n"
...
Value gettransaction(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
                ...
                "\nResult:\n"
                "{\n"
                "  \"amount\" : x.xxx,        (numeric) The transaction amount in IZZY\n"
                ...
                "      \"address\" : \"izzyaddress\",   (string) The izzy address involved in the transaction\n"
                "      \"category\" : \"send|receive\",    (string) The category, either 'send' or 'receive'\n"
                "      \"amount\" : x.xxx                  (numeric) The amount in IZZY\n"
                "      \"vout\" : n,                       (numeric) the vout value\n"
                "    }\n"
                "    ,...\n"
                "  ],\n"
...
Value encryptwallet(const Array& params, bool fHelp)
{
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() != 1))
        throw runtime_error(
                ...
                "\nNow we can so something like sign\n" + HelpExampleCli("signmessage", "\"izzyaddress\" \"test message\"") +
    ...
    StartShutdown();
    return "wallet encrypted; izzy server stopping, restart to run with encrypted wallet. The keypool has been flushed, you need to make a new backup.";
}
...
Value getwalletinfo(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
                ...
                "  \"balance\": xxxxxxx,         (numeric) the total IZZY balance of the wallet\n"
