...
bool CoinMinter::ProcessBlockFound(CBlock* block, CReserveKey& reservekey) const
{
    ...
    // Found a solution
    {
        ...
        if (block->hashPrevBlock != chain_.Tip()->GetBlockHash())
            return error("IZZYMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, block))
        return error("IZZYMiner : ProcessNewBlock, block not accepted");
    ...
    CReserveKey& reserveKey) const
{
    ...
    LogPrintf("Running IZZYMiner with %u transactions in block (%u bytes)\n", block->vtx.size(),
