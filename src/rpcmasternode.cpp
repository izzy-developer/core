...
Value allocatefunds(const Array& params, bool fHelp)
{
	if (fHelp || params.size() != 3)
		throw std::runtime_error(
			...
			"      <future>     (numeric, required) amount of izzy funded will also be accepted for partially funding master nodes and other purposes.\n"
...
{
	std::string ret = indent + "\"address\": \"address\",    (string) Masternode IZZY address\n";
...
Value listmasternodes(const Array& params, bool fHelp)
{
    ...
    if (fHelp || (params.size() > 1))
        throw std::runtime_error(
            ...
            "    \"addr\": \"addr\",      (string) Masternode IZZY address\n"
...
Value getmasternodestatus (const Array& params, bool fHelp)
{
    if (fHelp || (params.size() != 0))
        throw std::runtime_error(
            ...
            "  \"addr\": \"xxxx\",        (string) IZZY address for masternode payments\n"
...
Value getmasternodewinners (const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 3)
        throw std::runtime_error(
            ...
            "    \"winner\": {\n"
            "      \"address\": \"xxxx\",    (string) IZZY MN Address\n"
