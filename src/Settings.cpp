...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "izzy.conf"));
...
void CopyableSettings::ReadConfigFile()
{
    ...
    if (!streamConfig.good()) {
        // Create empty izzy.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override izzy.conf
