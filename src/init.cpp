...
// threads have exited.
//
// Note that if running -daemon the parent process returns from InitializeIzzy
...
void PrepareShutdown()
{
    ...
    /// Note: Shutdown() must be able to handle cases in which InitializeIzzy() failed part of the way,
    /// for example if the data directory was found to be locked.
    /// Be sure that anything that writes files or flushes caches only does this if the respective
    /// module was initialized.
    RenameThread("izzy-shutoff");
...
/** Sanity checks
 *  Ensure that IZZY is running in a usable environment with all
...
bool EnableWalletFeatures()
{
    if (settings.GetBoolArg("-salvagewallet", false)) {
        ...
        if (settings.SoftSetBoolArg("-rescan", true))
            LogPrintf("InitializeIzzy : parameter interaction: -salvagewallet=1 -> setting -rescan=1\n");
    }

    // -zapwallettx implies a rescan
    if (settings.GetBoolArg("-zapwallettxes", false)) {
        if (settings.SoftSetBoolArg("-rescan", true))
            LogPrintf("InitializeIzzy : parameter interaction: -zapwallettxes=<mode> -> setting -rescan=1\n");
    }
...
{
    // Make sure only a single IZZY process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(translate("Cannot obtain a lock on data directory %s. IZZY Core is probably already running."), datadir));
    return true;
}
...
void ClearFoldersForResync()
{
    ...
    // We delete in 4 inizzydual steps in case one of the folder is missing already
    try {
...
bool TryToLoadBlocks(bool& fLoaded, std::string& strLoadError)
{
    ...
    try {
        ...
        // IZZY: load previous sessions sporks if we have them.
...
bool CreateNewWalletIfOneIsNotAvailable(std::string strWalletFile, std::ostringstream& strErrors)
{
    ...
    if (nLoadWalletRet != DB_LOAD_OK) {
        if (nLoadWalletRet == DB_CORRUPT)
            ...
            strErrors << translate("Error loading wallet.dat: Wallet requires newer version of IZZY Core") << "\n";
        else if (nLoadWalletRet == DB_NEED_REWRITE) {
            strErrors << translate("Wallet needed to be rewritten: restart IZZY Core to complete") << "\n";
...
bool InitializeIzzy(boost::thread_group& threadGroup)
{
// ********************************************************* Step 1: setup
#ifdef _MSC_VER
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (fDisableWallet) {
        if (settings.SoftSetBoolArg("-staking", false))
            LogPrintf("InitializeIzzy : parameter interaction: wallet functionality not enabled -> setting -staking=0\n");
    }
    ...
    if (!VerifyCriticalDependenciesAreAvailable())
        return InitError(translate("Initialization sanity check failed. IZZY Core is shutting down."));
...
// ********************************************************* Step 8: load wallet
    ...
    if (pwalletMain)
        GenerateIzzy(settings.GetBoolArg("-gen", false), pwalletMain, settings.GetArg("-genproclimit", 1));
#endif
