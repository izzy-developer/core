...
#if defined(NDEBUG)
#error "IZZY cannot be compiled without assertions."
#endif
...
bool AcceptToMemoryPool(CTxMemPool& pool, CValidationState& state, const CTransaction& tx, bool fLimitFree, bool* pfMissingInputs, bool ignoreFees)
{
    ...
    if (tx.IsCoinBase())
        return state.DoS(100, error("%s: : coinbase as inizzydual tx",__func__),
                         REJECT_INVALID, "coinbase");

    //Coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
        return state.DoS(100, error("%s: coinstake as inizzydual tx",__func__),
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex* const pindexPrev)
{
    ...
    // All blocks on IZZY later than the genesis block must be at least version 3.
