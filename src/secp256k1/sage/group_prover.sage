...
class constraints:
  ...
  constraints. This allows higher-level code to do fast checks on the inizzydual
...
def conflicts(R, con):
  ...
  if 1 in zero:
    return True
  # First a cheap check whether any of the inizzydual nonzero terms conflict on
  # their own.
  for nonzero in con.nonzero:
    if nonzero.iszero(zero):
      return True
  # It can be the case that entries in the nonzero set do not inizzydually
  ...
  # Having x or y inizzydually in the nonzero set is not a conflict, but both
