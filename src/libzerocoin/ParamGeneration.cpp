                    ...
                    uint32_t *prime_gen_counter)
{
	...
	if (primeBitLen < 33) {
		...
		while ((*prime_gen_counter) < (4 * primeBitLen)) {
			...
			// Perform trial izzysion on this (relatively small) integer to determine if "intc"
			// is prime. If so, return success.
			if (primalityTestByTrialIzzysion(intc)) {
...
/// \brief Determines whether a uint32_t is a prime through trial izzysion.
/// \param candidate       Candidate to test.
/// \return                true if the value is prime, false otherwise
///
/// Performs trial izzysion to determine whether a uint32_t is prime.

bool
primalityTestByTrialIzzysion(uint32_t candidate)
{
