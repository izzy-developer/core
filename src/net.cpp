...
void CNode::copyStats(CNodeStats& stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (IZZY users should be well used to small numbers with many decimal places by now :)
...
void ThreadMapPort()
{
    ...
    if (r == 1) {
        ...
        string strDesc = "IZZY " + FormatFullVersion();
...
bool BindListenPort(const CService& addrBind, string& strError, bool fWhitelisted)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR) {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(translate("Unable to bind to %s on this computer. IZZY Core is probably already running."), addrBind.ToString());
        else
...
void SetNetworkingParameters()
{
    if (settings.ParameterIsSet("-bind") || settings.ParameterIsSet("-whitebind")) {
        ...
        if (settings.SoftSetBoolArg("-listen", true))
            LogPrintf("InitializeIzzy : parameter interaction: -bind or -whitebind set -> setting -listen=1\n");
    }
    ...
    if (settings.ParameterIsSet("-connect") && settings.GetMultiParameter("-connect").size() > 0) {
        ...
        if (settings.SoftSetBoolArg("-dnsseed", false))
            LogPrintf("InitializeIzzy : parameter interaction: -connect set -> setting -dnsseed=0\n");
        if (settings.SoftSetBoolArg("-listen", false))
            LogPrintf("InitializeIzzy : parameter interaction: -connect set -> setting -listen=0\n");
    }
    ...
    if (settings.ParameterIsSet("-proxy")) {
        ...
        if (settings.SoftSetBoolArg("-discover", false))
            LogPrintf("InitializeIzzy : parameter interaction: -proxy set -> setting -discover=0\n");
    }

    if (!settings.GetBoolArg("-listen", true)) {
        // do not map ports or try to retrieve public IP when not listening (pointless)
        if (settings.SoftSetBoolArg("-upnp", false))
            LogPrintf("InitializeIzzy : parameter interaction: -listen=0 -> setting -upnp=0\n");
        if (settings.SoftSetBoolArg("-discover", false))
            LogPrintf("InitializeIzzy : parameter interaction: -listen=0 -> setting -discover=0\n");
        if (settings.SoftSetBoolArg("-listenonion", false))
            LogPrintf("InitializeIzzy : parameter interaction: -listen=0 -> setting -listenonion=0\n");
    }

    if (settings.ParameterIsSet("-externalip")) {
        // if an explicit public IP is specified, do not try to find others
        if (settings.SoftSetBoolArg("-discover", false))
            LogPrintf("InitializeIzzy : parameter interaction: -externalip set -> setting -discover=0\n");
    }
