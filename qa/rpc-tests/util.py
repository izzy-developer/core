...
def initialize_datadir(dirname, n):
    ...
    if not os.path.isdir(regtest):
        os.makedirs(regtest)
    with open(os.path.join(datadir, "izzy.conf"), 'w') as f:
...
def start_node(i, dirname, extra_args=None, mn_config_lines=[], rpchost=None):
    """
    Start a izzyd and return RPC connection to it
    """
    ...
    binary.append(os.getenv("BITCOIND", "izzyd"))
    args = binary + ["-datadir="+datadir, "-keypool=1", "-discover=0", "-rest"]
    if extra_args is not None: args.extend(extra_args)
    bitcoind_processes[i] = subprocess.Popen(args)
    devnull = open("/dev/null", "w+")
    subprocess.check_call([ os.getenv("BITCOINCLI", "izzy-cli"), "-datadir="+datadir] +
...
def start_nodes(num_nodes, dirname, extra_args=None, mn_config_lines=None, rpchost=None):
    """
    Start multiple izzyds, return RPC connections to them
    """
