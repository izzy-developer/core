...
# Copyright (c) 2020 The IZZY developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.
"""Run regression test suite.

This module calls down into inizzydual test cases via subprocess. It will
forward all unrecognized arguments onto the inizzydual test scripts.
...
def main():
    ...
    parser = argparse.ArgumentParser(add_help=False,
                                     ...
                                     description=__doc__,
                                     epilog='''
    Help text and arguments for inizzydual test script:''',
    ...
    enable_izzyd = config["components"].getboolean("ENABLE_IZZYD")
    ...
    test_list = []
    if tests:
        # Inizzydual tests have been specified. Run specified tests that exist
...
def run_tests(*, test_list, src_dir, build_dir, tmpdir, jobs=1, args=None, failfast=False, use_term_control):
    ...
    # Warn if izzyd is already running
    try:
        # pgrep exits with code zero when one or more matching processes found
        if subprocess.run(["pgrep", "-x", "izzyd"], stdout=subprocess.DEVNULL).returncode == 0:
            print("%sWARNING!%s There is already a izzyd process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    except OSError:
        # pgrep not supported
        pass
