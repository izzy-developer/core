...
def main(argv):
    ...
    parser.add_argument('--prefix', dest='prefix', help='path to folder where izzy-cli is located', default='.')
    ...
    executableName = prefix + '/izzy-cli'
    print executableName

    if os.path.isfile(executableName) == False:
        print 'Cannot find izzy-cli, consider using --prefix'
        sys.exit(0)

    try:
        json.loads(executeProcess([executableName, 'getinfo']))
    except ValueError:
        print 'Izzy-cli is not running, or it is in wrong dir'
        sys.exit(0)
